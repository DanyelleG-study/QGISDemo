	...
	: QMainWindow(parent)
{
	...
	addAttributeToolsAction();//添加属性表按钮
	initMapTool();//初始化编辑工具
...
void qgisDemo::addEditToolsAction()
{
	...
	//m_ActionAddFeature->setEnabled(false);
	//m_ActionSaveLayerEdits->setEnabled(false);
...
void qgisDemo::addAttributeToolsAction()
{
	m_ActionAttributeForm = new QAction(QStringLiteral("属性表"));
	m_ActionAttributeForm->setIcon(QgsApplication::getThemeIcon(QStringLiteral("mActionOpenTable.svg")));
	ui.mainToolBar->addAction(m_ActionAttributeForm);
	m_ActionAttributeForm->setEnabled(false);
	connect(m_ActionAttributeForm, &QAction::triggered, this, &qgisDemo::showAttributteForm);
}
void qgisDemo::showAttributteForm()
{
	QgsVectorLayer *vec = qobject_cast<QgsVectorLayer*>(activeLayer());
	if (vec)
	{
		QgsDualView *dualView = new QgsDualView(nullptr);
		dualView->setWindowTitle(QStringLiteral("属性表"));
		dualView->init(vec, m_MapCanvas);
		QgsAttributeTableConfig config = vec->attributeTableConfig();
		dualView->setAttributeTableConfig(config);
		dualView->show();
	}
}
...
void qgisDemo::addVecLayer()
{
	...
	m_ActionAttributeForm->setEnabled(true);
	m_ActionToggleEditing->setEnabled(true);
}